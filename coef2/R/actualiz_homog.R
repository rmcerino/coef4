...
valor_actualizado <- function(tc_act, datos, elasticidad){
  ...
  datos$vm2_2019 <- (1 + datos$var_tc * datos$elasticidad) * datos$valor_m2
  datos$vm2_2019 <<- datos$vm2_2019
...
func_homog <- function (form, datos, dist_lw, p_valor, parcelas) {
  ...
  datos$m2_coef <- datos$vm2_2019/datos$coef

  datos$m2_coef <<- datos$m2_coef

  vh <- summary(datos$vm2_2019/datos$coef)
